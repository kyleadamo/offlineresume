    ...
    summary: () => summary ? (
      <div key="summary" data-section="summary" className={`mb-6 ${sectionClass}`}>
        <div className="border-[2px] border-foreground">
          <div className="bg-yellow-300 text-foreground px-3 py-1 text-xs font-black uppercase tracking-widest border-b-[2px] border-foreground">About</div>
            ...
            <div key={exp.id} data-pdf-section className="border-[2px] border-foreground p-3">
              <div className="flex justify-between items-baseline flex-wrap gap-2">
                ...
                {(exp.startDate || exp.endDate) && (
                  <span className="text-xs font-bold uppercase shrink-0">{exp.startDate}{exp.endDate ? ` — ${exp.endDate}` : ''}</span>
                )}
              </div>
              {exp.bullets.filter(Boolean).length > 0 && (
                <ul className="mt-2 space-y-1 text-muted-foreground">
                  {exp.bullets.filter(Boolean).map((b, i) => (
                    <li key={i} className="flex gap-2"><span className="shrink-0 font-bold text-foreground">→</span><span>{b}</span></li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <div className="space-y-3">
          {education.map((edu) => (
            <div key={edu.id} data-pdf-section className="border-[2px] border-foreground p-3">
              ...
              {(edu.startDate || edu.endDate) && <div className="text-xs font-bold uppercase mt-1">{edu.startDate}{edu.endDate ? ` — ${edu.endDate}` : ''}</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      {profile.name && (
        <div data-section="profile" className={`mb-6 ${sectionClass}`}>
          <div className="flex items-center gap-4">
            {profile.photo && <img src={profile.photo} alt={profile.name} className="w-16 h-16 object-cover shrink-0 border-[3px] border-foreground" />}
            <div>
              <h2 className="text-3xl font-black uppercase tracking-tight">{profile.name}</h2>
              {resume.targetRole && <p className="text-sm font-bold uppercase tracking-widest mt-1 bg-foreground text-background inline-block px-2 py-0.5">{resume.targetRole}</p>}
            </div>
          </div>
