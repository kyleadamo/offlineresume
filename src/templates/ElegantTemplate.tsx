...
const ElegantTemplate = ({ resume }: TemplateProps) => {
  ...
  const newRenderers = createNewSectionRenderers(resume, {
    sectionClass,
    headingClass: '',
    renderHeading: (label) => <h3 className="text-[11px] font-medium uppercase tracking-[0.25em] text-muted-foreground mb-4">{label}</h3>,
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => summary ? (
      <div key="summary" data-section="summary" className={`mb-7 ${sectionClass}`}>
        <p className="text-muted-foreground leading-relaxed">{summary}</p>
      </div>
    ) : null,
    experience: () => experience.length > 0 ? (
      <div key="experience" data-section="experience" className={`mb-7 ${sectionClass}`}>
              ...
              {exp.bullets.filter(Boolean).length > 0 && (
                <ul className="mt-2 space-y-1 text-muted-foreground">
                  {exp.bullets.filter(Boolean).map((b, i) => (
                    <li key={i} className="flex gap-2"><span className="shrink-0 mt-0.5 text-xs">◦</span><span>{b}</span></li>
                  ))}
                </ul>
              )}
            </div>
    ...
    education: () => education.length > 0 ? (
      <div key="education" data-section="education" className={`mb-7 ${sectionClass}`}>
    ...
    skills: () => skills.length > 0 ? (
      <div key="skills" data-section="skills" className={`mb-7 ${sectionClass}`}>
    ...
    projects: () => projects.length > 0 ? (
      <div key="projects" data-section="projects" className={`mb-7 ${sectionClass}`}>
      ...
      {profile.name && (
        <div data-section="profile" className={`mb-8 ${sectionClass}`}>
          <div className="flex items-center gap-5">
            {profile.photo && <img src={profile.photo} alt={profile.name} className="w-14 h-14 rounded-full object-cover shrink-0" />}
            <div>
              <h2 className="text-2xl font-light tracking-wide uppercase">{profile.name}</h2>
              {resume.targetRole && <p className="text-xs text-muted-foreground tracking-widest uppercase mt-1">{resume.targetRole}</p>}
            </div>
          </div>
